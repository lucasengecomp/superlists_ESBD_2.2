...
class NewItemTets(TestCase):
	def test_can_save_a_POST_request_to_an_existing_list(self):
		...
		self.client.post(
			...
			data={'item_text': 'A new item for an existing list', 'item_priority': "Prioridade Alta"}
		)

		self.assertEqual(Item.objects.count(), 1)
		new_item = Item.objects.first()
		self.assertEqual(new_item.text, 'A new item for an existing list')
		self.assertEqual(new_item.priority, 'Prioridade Alta')
...
class ListViewTest(TestCase):
	...
	def test_displays_only_items_for_that_list(self):
		...
		other_list = List.objects.create()
		Item.objects.create(text='other list item 1', priority='priority 1',  list=other_list)
		Item.objects.create(text='other list item 2', priority='priority 2',  list=other_list)
